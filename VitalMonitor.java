...
public class VitalMonitor {
    ...
    private static InetAddress getIPAddressOfComputer() {
        ...
        try {
            ...
            socket.connect(InetAddress.getByName("8.8.8.8"), 80); // connecting with dns.google
            ip_address = InetAddress.getByName(socket.getLocalAddress().getHostAddress()); // setting up the ip address choosen by the kernal
            socket.close();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private static byte[] convertToByteArray(Monitor monitor) {
        ...
        try {
            ...
            oos.writeObject(monitor); // writing the moniter object into the buffer
            data = bos.toByteArray(); // copying the object as a byte array from the buffer
        } catch (IOException e) {
            e.printStackTrace();
        }
        return data;
    ...
    private static DatagramSocket createBroadcastSocket() {
        ...
        try {
            ...
            socket.setBroadcast(true); // Enabling SO_BROADCAST
    ...
    private static void customDelayInSeconds(int seconds) {
        try {
            TimeUnit.SECONDS.sleep(seconds); //Thread.sleep() 
    ...
    private static void broadcastMonitorIdentity(byte[] data, InetAddress ipAddress, int broadcastPort, DatagramSocket broadcastSocket) {
        DatagramPacket packet = new DatagramPacket(data, data.length, ipAddress, broadcastPort); // creating a data packet
        try {
